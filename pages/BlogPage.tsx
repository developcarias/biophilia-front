    ...
    <>
      ...
      <div className="bg-white py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            {/* Featured Post */}
            <div className="mb-16">
                <h2 className="text-3xl font-bold text-brand-green-dark mb-4 border-b pb-2">{t('featuredPost')}</h2>
                <NavLink to={`/blog/${featuredPost.slug}`} className="group block rounded-lg shadow-xl overflow-hidden relative text-white">
                ...
                <div className="flex flex-wrap justify-center -m-4">
                    {otherPosts.map((post) => (
                        <div key={post.id} className="w-full md:w-1/2 lg:w-1/3 p-4">
                            <NavLink to={`/blog/${post.slug}`} className="group bg-brand-green-light rounded-lg shadow-lg overflow-hidden flex flex-col transition-transform transform hover:-translate-y-2 hover:shadow-2xl h-full">
